...
import oop_class.Seminar5.chars.Marksman;
import oop_class.Seminar5.chars.Monk;
import oop_class.Seminar5.chars.Peasant;
import oop_class.Seminar5.chars.Pikeman;
import oop_class.Seminar5.chars.Sniper;
import oop_class.Seminar5.chars.Thief;
import oop_class.Seminar5.chars.Wizard;
...
public class Main {
    ...
    public static void init() {
        ...
        for (int i = 0; i < TEAM_SIZE; i++) {
            switch (new Random().nextInt(4)) {
                case 0:
                    lightSide.add(new Peasant(lightSide, x++, y, 5));
                    break; 
                case 1:
        ...
        for (int i = 0; i < TEAM_SIZE; i++) {
            switch (new Random().nextInt(4)) {
                case 0: 
                    darkSide.add(new Peasant(darkSide, x++, y, 5));
                    break;
                case 1: 
    ...
    //         if (unit.getStatus() == "Dead") deadDark.add(String e);
    //     }
    //     if (deadDark.total == 10) System.out.println("GAME OVER"); 
        
    //     for (Base unit : Main.lightSide) {
    //         if (unit.getStatus() == "Dead") deadLight.add(String e);
    //     }
    //     if (deadLight.total == 10) System.out.println("GAME OVER");
    //     System.exit(0);
    // } 
}
